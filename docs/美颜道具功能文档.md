...
```
美颜bundle默认的aitype为FUAITYPE_FACEPROCESSOR，避免了美颜和贴纸同开时跑两份。逻辑上要注意上，进入仅美颜场景时，参数“landmarks_type”切换为 FaceUnity.FUAITYPE_FACELANDMARKS75，底层会仅跑nn75。如果进入美颜+其他内容的场景，需要设置为 FaceUnity.FUAITYPE_FACEPROCESSOR，避免跑两份。
```
