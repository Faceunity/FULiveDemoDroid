 ...
 * 可调节的 landmarks
 */
public class AdjustProgramLandmarks extends Program {
    ...
    private final float[] mMvpMtx = new float[16];
    ...
    public void drawFrame(int x, int y, int width, int height, float[] mvpArea) {
        if (mvpArea == null) {
            System.arraycopy(mMvpMtx, 0, mvp, 0, mvp.length);
        } else {
            Matrix.multiplyMM(mvp, 0, mvpArea, 0, mMvpMtx, 0);
        }
    ...
    public void refresh(float[] landmarksData, int width, int height) {
        if (mWidth != width || mHeight != height) {
            ...
            Matrix.multiplyMM(mMvpMtx, 0, rotateMtx, 0, orthoMtx, 0);

            mWidth = width;
            mHeight = height;
        }
        if (mLandmarksData == null || mLandmarksData.length != landmarksData.length) {
            mLandmarksData = new float[landmarksData.length];
        }
        System.arraycopy(landmarksData, 0, mLandmarksData, 0, landmarksData.length);
        updateVertexArray(mLandmarksData);
    }
